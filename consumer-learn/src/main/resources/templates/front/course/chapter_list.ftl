...
<div id="main">
    <div class="course-infos">
        <div class="w pr">
            ...
            <div class="statics clearfix">
                ...
                <#assign count=0/>
                <#assign flag =0 />
                <#if userVideoList?exists>
                <#list userVideoList?sort_by("modifyDate")?reverse as userVideo>
                    <#if "${count}"?number==0 && "${(userFront.id)!}" == "${(userVideo.userId)!}">
                        <#assign flag = userVideo.videoId />
                    </#if>
                    ...
                    <a href="${base}/video/videoView?id=${flag}" style="">继续学习</a>
                    <em></em>
                ...
                <div class="static-item l">
                    ...
                    <#list videoList as video>
                        <#if video.id == flag>
                            <span class="meta-value smale-value" title="SpringBoot介绍">${video.serialNumber}&nbsp;&nbsp;${video.videoName}</span>
                        </#if>
    ...
    <div class="course-info-main clearfix w">
        <div class="content-wrap">
                            ...
                            <#list videoList as video>
                                <#if video.chapterId == chapter.id>
                                    <li data-media-id="13589">
                                        ...
                                        ${video.videoName} (${(video.videoDuration?number/60)?int}:${video.videoDuration?number%60})
                                            <#if userVideoList?exists>
                                                <#if flag == video.id><font color="green"><b>您上次学到这里，点击继续学习</b></font></#if>
                                            <#list userVideoList as userVideo>
                                                <#if userVideo.videoId == video.id>
                                                    &nbsp;&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
                                                    <b style="color: red;">您的学习进度：${userVideo.progress?int / video.videoDuration?int *100 }%</b>
                                                    <#if userVideo.progress?int == video.videoDuration?int>
                                                        <i style="color: green;"class="icon-check"></i>您已经学完这节视频，看看下节吧。
                                                    </#if>
                                                </#if>
                                            </#list>
                                            </#if>
            ...
            <!--进入考试-->
        <#if testpaper?size != 0>
            <div class="entertest_btn">
                <#--<a href="${base}/test/exam?id=${RequestParameters['id']}"></a>-->
                <a href="#"><div class="enter_test" onclick="entertest()"><span class="exam_test">进入考试</span></div></a>
            </div>

        <#else>
            <div class="entertest_btn">
                <div class="enter_test"><span class="exam_test">考试未开</span></div>
            </div>
        </#if>
...
<script  type="text/javascript">
    function entertest() {
        var con;
        con=confirm("考试时间为30分钟，点击确定将进入考试并计时"); //在页面上弹出对话框
        if(con==true){
            window.location.href="${base}/test/exam?id=${RequestParameters['id']}";
        }
        else {alert("您已取消考试");}
    }
</script>